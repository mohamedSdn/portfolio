...
export const cat = ({ currentDirectory, setQueryList }: IAppContext, fullCommand: string, extraParams: string[]) => {
    if (extraParams.length > 1) {
        setQueryList(prev => [...prev, { directory: currentDirectory, command: fullCommand, result: "'cat' takes exactly one argument" }]);
        return;
    }
    ...
    try {
        ...
        const file = result.find(file => !file.isDirectory && file.name === directories.at(-1));
        if (!file) {
