...
export const formatLsOutput = (input: IFile[]) => {
    const filesToDisplay = input
        .map((file) => {
            return `<span style="margin-right: 1rem; ${
                file.isDirectory ? "color: #6da5ea;" : ""
            }">${file.name}</span>`;
        })
        .join(" ");
...
export const isCharAcceptable = (char: string) =>
    ACCEPTABLE_CHAR_REGEX.test(char);
